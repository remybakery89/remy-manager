...
(function(){
  ...
  function actionPanel(){
    const exp=expiryData();const groups={};exp.forEach(b=>{if(!groups[b.ingredientId])groups[b.ingredientId]={ingredient:b.ingredient,items:[]};groups[b.ingredientId].items.push(b);});
    const suggestions=productSuggestions();
    return `<div class="card" style="margin-top:16px"><div class="section-title">⏳ Nguyên liệu cần ưu tiên sử dụng</div><div style="font-size:12px;color:var(--muted);margin:-8px 0 12px">Hết hạn và gần hết hạn được đưa lên trước. Bấm vào nguyên liệu để xem công thức và sản phẩm liên quan.</div>${Object.values(groups).map(g=>{const days=Math.min(...g.items.map(x=>x.days));const qty=g.items.reduce((s,x)=>s+Number(x.qty||0),0);return `<div class="list-item row" style="cursor:pointer" onclick="r4ExpiryDetail('${g.ingredient.id}')"><div><b>${g.ingredient.name}</b><div style="font-size:12px;color:var(--muted)">${num(qty)} ${g.ingredient.unit} · ${g.items.length} lô</div></div><span class="badge ${days<0?'danger':'warn'}">${days<0?'Đã hết hạn':`Còn ${days} ngày`}</span></div>`;}).join('')||'<div class="empty">Không có nguyên liệu hết hạn/gần hết hạn. 🎉</div>'}</div><div class="card" style="margin-top:16px"><div class="section-title">🚀 Sản phẩm nên ưu tiên đẩy bán</div><div style="font-size:12px;color:var(--muted);margin:-8px 0 12px">Xếp theo lượng sử dụng và số loại nguyên liệu đang hết hạn/gần hết hạn.</div>${suggestions.map((x,i)=>`<div class="list-item row"><div><b>${i<2?'🔥 ':''}${x.p.name}</b><div style="font-size:12px;color:var(--muted)">${x.hits.length} loại nguyên liệu cần ưu tiên</div></div><span class="badge ${i<2?'danger':i<5?'warn':'info'}">${i<2?'Rất cao':i<5?'Cao':'Trung bình'}</span></div>`).join('')||'<div class="empty">Chưa có sản phẩm cần ưu tiên.</div>'}</div>`;
  }
