...
(function(){
  ...
  function operationalOverview(){
    ...
    const needs={};
    function addRecipe(r,m=1,seen=[]){if(!r||seen.includes(r.id))return;const next=[...seen,r.id];(r.lines||[]).forEach(l=>{if(l.kind==='recipe')addRecipe(db.recipes.find(x=>x.id===l.recipeId),m*(Number(l.qty)||0),next);else if(l.ingredientId){const q=Number(l.qty)||0,w=Math.min(99.99,Math.max(0,Number(l.wastePct)||0))/100;needs[l.ingredientId]=(needs[l.ingredientId]||0)+m*(w>=1?q:q/(1-w));}});}
    plans.forEach(p=>{const qty=Number(p.qty)||0;if(p.recipeId)addRecipe(db.recipes.find(r=>r.id===p.recipeId),qty);else if(p.productId){const pr=db.products.find(x=>x.id===p.productId);(pr?.components||[]).forEach(c=>addRecipe(db.recipes.find(r=>r.id===c.recipeId),qty*(Number(c.qty)||0)));}});
    shortageIngredients=Object.entries(needs).filter(([id,q])=>{const stock=db.batches.filter(b=>b.ingredientId===id&&Number(b.qty)>0&&(!b.expiry||b.expiry>=today())).reduce((s,b)=>s+Number(b.qty||0),0);return Number(q)>stock;}).length;
    const suggestions=typeof r4ProductSuggestions==='function'?r4ProductSuggestions().length:0;
    const item=(icon,title,value,sub,page,kind)=>`<div class="card" style="cursor:pointer" onclick="go('${page}')"><div class="row"><div><div class="label">${icon} ${title}</div><div class="value">${value}</div><div class="sub">${sub}</div></div><span class="badge ${kind}">Xem</span></div></div>`;
    return `<div class="card" style="margin-top:16px"><div class="section-title">Tổng quan vấn đề cần chú ý</div><div style="font-size:12px;color:var(--muted);margin:-8px 0 12px">Bổ sung nhanh vào Dashboard hiện tại — bấm từng mục để xử lý chi tiết.</div><div class="grid stats">${item('🔴','Hết hạn',expiry,'lô cần xử lý','alerts','danger')}${item('🟠','Sắp hết hạn',expiring,'lô trong 7 ngày','alerts','warn')}${item('🟡','Thiếu nguyên liệu',shortageIngredients,'nguyên liệu cho kế hoạch','production','warn')}${item('🔵','Tồn kho thấp',lowStock,'nguyên liệu dưới mức theo dõi','inventory','info')}${item('🚀','Sản phẩm nên đẩy',suggestions,'gợi ý từ nguyên liệu hạn','alerts','ok')}</div></div>`;
  }
