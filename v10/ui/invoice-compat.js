...
(function(){
  ...
  async function renderInvoiceCanvas(oid){
    ...
    let H=360+rows.reduce((n,r)=>n+Math.max(36,r.lines.length*24)+18,0)+190;
    H=Math.max(1231,H);
    ...
    if(logo){ctx.drawImage(logo,pad,y,105,105);drawText(ctx,s.storeName,pad+125,y+4,contentW-125,34,{font:'800 30px Arial'});if(s.phone)drawText(ctx,'SĐT: '+s.phone,pad+125,y+42,contentW-125,23);if(s.showAddress&&s.address)drawText(ctx,s.address,pad+125,y+68,contentW-125,23);y+=120}
    else{y=drawText(ctx,s.storeName,pad,y,contentW,36,{font:'800 30px Arial'});if(s.phone)y=drawText(ctx,'SĐT: '+s.phone,pad,y+4,contentW,23);if(s.showAddress&&s.address)y=drawText(ctx,s.address,pad,y+2,contentW,23);y+=10}
    ctx.strokeStyle='#111';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(pad,y);ctx.lineTo(W-pad,y);ctx.stroke();y+=20;
    y=drawText(ctx,s.title,W/2,y,contentW,34,{font:'800 30px Arial',align:'center'});y+=18;
    if(s.showOrderCode)y=drawText(ctx,`Mã đơn: #${o.id.slice(-6).toUpperCase()}`,pad,y,contentW,25,{font:'16px Arial'});
    if(s.showDate)y=drawText(ctx,`Ngày bán: ${new Date(o.createdAt||o.date).toLocaleString('vi-VN')}`,pad,y,contentW,25);
    if(s.showCustomer)y=drawText(ctx,`Khách hàng: ${c?.name||'Khách lẻ'}${s.showPhone&&c?.phone?' · '+c.phone:''}`,pad,y,contentW,25);
    y+=12;line(ctx,pad,y,W-pad);y+=14;
    const xQty=W-310,xPrice=W-205,xTotal=W-pad;ctx.font='800 16px Arial';ctx.fillStyle='#111';ctx.textAlign='left';ctx.fillText('STT / Sản phẩm',pad,y);ctx.textAlign='right';ctx.fillText('SL',xQty,y);if(s.showItemUnitPrice)ctx.fillText('Giá',xPrice,y);if(s.showItemTotal)ctx.fillText('T.Tiền',xTotal,y);y+=26;line(ctx,pad,y,W-pad);y+=10;
    ...
    if(s.note){y=drawText(ctx,s.note,pad,y,contentWidth,20,{font:'13px Arial',color:'#555'});y+=10}if(s.showFooter){y+=18;drawText(ctx,s.footer,W/2,y,contentW,24,{font:'16px Arial',align:'center'});}
    return canvas;
  }
