...
(function(){
  ...
  window.unitOptions=function(selected){return COMMON.map(u=>`<option value="${u}" ${String(u)===String(selected)?'selected':''}>${u}</option>`).join('');};
  ...
  window.ingredientModal=function(editId){
    ...
    if(original&&x.packUnit&&x.unit&&x.packUnit!==x.unit){const converted=convertQty(displayPack,x.packUnit,x.unit);if(converted!=null)displayPack=converted;}
    ...
    const syncPackUnit=function(){
      ...
      if(next!==lastUnit){const converted=convertQty(pack,lastUnit,next);if(converted!=null){document.getElementById('fPack').value=String(Number(converted.toFixed(6)));}lastUnit=next;}
      updateIngredientUnitHint();
    };
    updateIngredientUnitHint();
    document.getElementById('fUnit')?.addEventListener('change',syncPackUnit);
  ...
  window.updateIngredientUnitHint=function(){
    const pack=+f('fPack')||0,price=+f('fPrice')||0,unit=f('fUnit'),el=document.getElementById('ingredientUnitHint');
    if(!el)return;el.innerHTML=`Một quy cách = <b>${num(pack)} ${unit}</b> · Giá chuẩn = <b>${money(pack?price/pack:0)}/${unit}</b>`;
  };
  ...
  window.saveIngredientV21=function(eid){
    ...
    const old=db.ingredients.find(i=>i.id===eid),oldUnit=old?standardPricePerUnit(old):0;
    ...
    const newUnit=standardPricePerUnit(x),priceChanged=!!old&&Math.abs(newUnit-oldUnit)>1e-12;
    if(eid&&old&&priceChanged){
      const delta=newUnit-oldUnit,items=impactedProducts(eid),list=items.map(p=>{const z=productCostBreakdown(p).find(y=>y.ingredientId===eid);return `${p.name}: ${delta>=0?'+':''}${money((z?.qty||0)*delta)}`;}).join('\n')||'Không có sản phẩm bị ảnh hưởng';
