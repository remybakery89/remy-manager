...
(function(){
  ...
  function setStatus(text,kind){
    ...
    const top=document.getElementById('v10OnlineStatus');
    if(top)top.textContent=text;
  }
  function showApp(){
    const app=document.querySelector('.app');if(app)app.style.display='flex';
  }
  function hideApp(){
  ...
  function jsonError(text){
    ...
    return new Error(t||'Máy chủ trả về dữ liệu không hợp lệ');
  }
  ...
  function queueSave(){
    ...
    saveChain=saveChain.then(async()=>{
      try{
        await pushSnapshot();
        state.pending=false;
        setStatus('Online · đã lưu máy chủ','ok');
      }catch(e){
        state.pending=false;
        setStatus('Lỗi kết nối máy chủ','danger');
        toast('❌ Chưa ghi được Google Sheets: '+(e.message||'Không xác định'));
        console.error('V10 save',e);
      }
    });
  }

  // All existing feature code calls save(). V10 turns that call into an immediate
  // online write; there is no local persistence fallback.
  ...
  function loginModal(message){
    openModal(`<h2>🔐 Đăng nhập F&B Manager</h2>
      ${message?`<div class="alert danger" style="margin-bottom:14px">${String(message).replace(/[<>]/g,'')}</div>`:''}
      <div class="form-grid">
        <div class="field full"><label>Tài khoản</label><input id="v10User" autocomplete="username"></div>
        <div class="field full"><label>Mật khẩu</label><input id="v10Pass" type="password" autocomplete="current-password" onkeydown="if(event.key==='Enter')v10DoLogin()"></div>
      </div>
      <div class="modal-actions"><button class="btn primary" id="v10LoginBtn" onclick="v10DoLogin()">Đăng nhập</button></div>`);
  }
  ...
  window.v9Login=v10DoLogin;
  window.v9LoginModal=loginModal;

  window.v9Logout=function(){
    state.user=null;state.branchId='MAIN';state.lastSync=null;state.pending=false;
  ...
  window.v9OpenAccount=function(){
    ...
    openModal(`<h2>☁️ Tài khoản & Online</h2>
      <div class="card" style="box-shadow:none">
        <div class="list-item row"><span>Đang đăng nhập</span><b>${esc(state.user.name||state.user.username)}</b></div>
        <div class="list-item row"><span>Vai trò</span><b>${esc(state.user.role||'—')}</b></div>
        <div class="list-item row"><span>Chi nhánh</span><b>${esc(state.user.branchName||state.branchId||'—')}</b></div>
      ...
      <div class="modal-actions"><button class="btn primary" onclick="v10SyncNow()">☁️ Lấy DATA mới</button><button class="btn danger" onclick="v9Logout()">Đăng xuất</button><button class="btn" onclick="closeModal()">Đóng</button></div>`);
  };
  ...
  window.v10SyncNow=async function(){
    ...
    state.busy=true;
    try{setStatus('Đang lấy DATA mới...','info');await pullOnline();setStatus('Online · DATA mới nhất','ok');toast('☁️ Đã lấy DATA mới nhất từ Google Sheets')}catch(e){setStatus('Lỗi kết nối máy chủ','danger');toast('❌ Không lấy được DATA: '+(e.message||'Không xác định'))}finally{state.busy=false}
  };
  ...
  window.v9SaveSettings=function(){
    state.branchId=(document.getElementById('v9Branch')?.value||state.branchId||'MAIN').trim()||'MAIN';
  ...
  window.v9TestConnection=async function(){
    ...
    try{
      ...
      if(!r.ok||data.ok===false)throw new Error(data.message||'Kết nối thất bại');
      setStatus(state.user?'Online · kết nối OK':'Kết nối OK','ok');toast('✅ Apps Script kết nối OK');
  ...
  window.v9SettingsCard=function(){
    return `<div class="card" style="margin-top:16px"><div class="section-title">☁️ Online & Đồng bộ</div>
      <div class="form-grid">
        <div class="field full"><label>Apps Script Web App URL</label><input value="${API}" readonly><div style="font-size:12px;color:var(--muted);margin-top:6px">Kết nối cố định. Không còn URL cũ, không lưu URL vào máy.</div></div>
        <div class="field"><label>Chi nhánh</label><input id="v9Branch" value="${esc(state.branchId||'MAIN')}"></div>
        <div class="field"><label>Trạng thái</label><div style="padding:10px 0"><span id="v9ConnectionBadge" class="badge ${state.user?'ok':'warn'}">${state.user?'Online · dữ liệu từ Google Sheets':'Chưa đăng nhập'}</span></div></div>
      </div>
      <div class="modal-actions"><button class="btn" id="v9TestBtn" onclick="v9TestConnection()">🔌 Kiểm tra kết nối</button><button class="btn primary" onclick="v9SaveSettings()">Lưu chi nhánh</button><button class="btn" onclick="v9OpenAccount()">Tài khoản</button><button class="btn" onclick="v10SyncNow()">☁️ Lấy DATA mới</button></div>
      <div style="margin-top:12px;font-size:12px;color:var(--muted)">Google Sheets là nguồn dữ liệu duy nhất. Thay đổi được ghi Online ngay; các thiết bị đang đăng nhập tự lấy DATA mới.</div></div>`;
  };

  // The old inline settings() remains part of the UI file. Override it only to add
  // the clean Online panel; all existing settings fields continue using db.settings.
  const oldSettings=window.settings;
  window.settings=function(){
    const base=typeof oldSettings==='function'?oldSettings():'';
    return base+(window.v9SettingsCard?window.v9SettingsCard():'');
  };

  function startPolling(){
    clearInterval(pollTimer);
    pollTimer=setInterval(async()=>{
      if(!state.user||state.busy||state.pending||!navigator.onLine||document.visibilityState==='hidden'||isModalOpen())return;
      try{await pullOnline();setStatus('Online · đã cập nhật','ok')}catch(e){console.warn('V10 pull',e)}
    },2500);
  }

  window.addEventListener('online',async()=>{
    state.online=true;
    if(state.user&&!state.busy&&!state.pending){try{await pullOnline();setStatus('Online · đã cập nhật','ok')}catch(e){console.warn('V10 reconnect',e)}}
    startPolling();
  });
  ...
  // Kill the legacy in-memory demo immediately. Runtime DATA starts empty and is
  // populated only after a successful login + server pull.
