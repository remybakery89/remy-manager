/* =========================================================
   ...
   Safe first step: manual sync only.
   - Sends local journal operations to Apps Script pushQueue
   - Removes only operations acknowledged by the server
   - Stores server conflicts locally
   - Does not replace the local database
   - Does not auto-overwrite another device
========================================================= */
(function(){
  'use strict';
  ...
  function getUrl(){
    ...
    const candidates=[
      ...
      localStorage.getItem('v9AppsScriptUrl')
    ].filter(Boolean);
    ...
    const found=inputs.find(i=>/^https:\/\/script\.google\.com\/macros\/s\/.+\/exec/i.test(i.value||''));
    return found?.value||'';
  }
  ...
  function syncStatus(){
    ...
    if(e){
      e.innerHTML=(online?'🟢 Online':'🟠 Offline')+' · '+q.length+' thay đổi đang chờ · '+c.length+' xung đột';
    }
  ...
  async function syncQueue(){
    ...
    if(navigator.onLine===false){
      toast('Đang offline — chưa thể đồng bộ');
      return;
    }
    ...
    if(!q.length){
      syncStatus();
      toast('Không có thay đổi đang chờ');
      return;
    }

    const url=getUrl();
    if(!url){
      toast('Chưa tìm thấy Apps Script Web App URL');
      return;
    }

    window.__v911SyncWorking=true;
    setButtonState(true);
    ...
    try{
      ...
      const newConflicts=(result.conflicts||[]).map(x=>({
        ...x,
        at:new Date().toISOString(),
        deviceId:read(META_KEY,{}).deviceId||''
      }));
      ...
      if(newConflicts.length){
        toast('Đã đồng bộ phần an toàn · còn '+newConflicts.length+' xung đột');
      }else{
        toast('Đồng bộ thành công · đã xử lý '+done.size+' thay đổi');
      }
    }catch(err){
      console.error('V9.11 sync',err);
      syncStatus();
      toast('Đồng bộ lỗi: '+(err?.message||'Không xác định'));
    }finally{
      window.__v911SyncWorking=false;
  ...
  function clearConflicts(){
    ...
    write(CONFLICT_KEY,[]);
    syncStatus();
    toast('Đã xóa danh sách xung đột');
  }

  function injectControls(){
    const card=document.getElementById('v910DataCard');
    if(!card)return false;
    if(document.getElementById('v911SyncButton')){
      syncStatus();
      return true;
    }
